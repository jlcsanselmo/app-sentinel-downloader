...
- **🗺️ Ingestão Espacial:** Upload de Shapefiles (`.zip`) e extração automática de Bounding Boxes (EPSG:4326) utilizando GeoPandas.
- **🛰️ Busca Inteligente (STAC API):** Integração com o catálogo `earth-search` da AWS para localizar passagens do satélite Sentinel-2 num raio temporal de 30 dias.
...
- **⚙️ Geoprocessamento em Nuvem:** Leitura e empilhamento de múltiplas bandas multiespectrais em um único arquivo `.tif` utilizando a biblioteca Rasterio.
